...
// Shared header/footer could be injected, but we use static HTML
// Cart & Wishlist state
let cart = JSON.parse(localStorage.getItem('gadget360-cart') || '[]');
let wishlist = JSON.parse(localStorage.getItem('gadget360-wishlist') || '[]');
...
function saveWishlist() {
  localStorage.setItem('gadget360-wishlist', JSON.stringify(wishlist));
  updateWishlistUI();
}
...
if (featuredEl) {
  featuredEl.innerHTML = featuredProducts.map(renderProductCard).join('');
}
    ...
    return `
      <div class="cart-item" style="display:flex;gap:1rem;padding:1rem;border-bottom:1px solid var(--border);">
        ...
        <div style="flex:1">
          ...
          <p style="color:var(--text-muted);font-size:0.9rem;">₹${p.price} × ${item.qty || 1}</p>
        </div>
...
document.querySelectorAll('.wishlist-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    // Placeholder - could open wishlist drawer
